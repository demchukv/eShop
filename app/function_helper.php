...
if (!function_exists('getCategoriesOptionHtml')) {
    function getCategoriesOptionHtml($categories, $selected_vals = null)
    {
        $html = "";

        foreach ($categories as $category) {
            $pre_selected = (!empty($selected_vals) && in_array($category['id'], $selected_vals)) ? "selected" : "";
            $category_name = htmlspecialchars_decode($category['name']);
            $html .= '<option value="' . $category['id'] . '" class="l' . $category['level'] . '" ' . $pre_selected . '  >' . e($category_name) . '</option>';
            if (!empty($category['children'])) {
                $html .= getCategoriesOptionHtml($category['children'], $selected_vals);
            }
        }
        return $html;
    }
...
function getVariantsValuesByPid($id, $status = [1])
{
    $varaint_values = [];
...
function fetchProduct($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = 20, $offset = NULL, $sort = 'p.id', $order = 'DESC', $return_count = NULL, $is_deliverable = NULL, $seller_id = NULL, $brand_id = NULL, $store_id = NULL, $is_detailed_data = 0, $type = '', $from_seller = 0, $show_unapproved = false)
{
    ...
    if (!empty($product)) {
        for ($i = 0; $i < count($product); $i++) {
            ...
            $product[$i]->name = outputEscaping($product[$i]->name);
            ...
            if ($product[$i]->deliverable_type != 'NONE' && $product[$i]->deliverable_type != 'ALL') {
                ...
                $zones = Zone::whereIn('id', $zone_ids)->get()->toArray();
                $zones = array_column($zones, 'name');
                $product[$i]->deliverable_zones = implode(",", $zones);
            } else {
                $product[$i]->deliverable_zones = '';
            }
            $product[$i]->category_name = (isset($product[$i]->category_name) && !empty($product[$i]->category_name)) ? outputEscaping($product[$i]->category_name) : '';
...
function fetchComboProduct($user_id = NULL, $filter = NULL, $id = NULL, $limit = NULL, $offset = NULL, $sort = 'p.id', $order = 'DESC', $return_count = NULL, $is_deliverable = NULL, $seller_id = NULL, $store_id = NULL, $category_id = '', $brand_id = '', $type = '', $from_seller = '')
{
    ...
    if (isset($filter['discount']) && !empty($filter['discount']) && $filter['discount'] != "") {
        ...
        if ($sort !== null || $order !== null && $sort !== 'p.price') {
            $product_query->orderBy('p.price', 'desc');
        }
    }
...
function validateStock($product_variant_ids, $qtns, $product_type = "")
{
    ...
    foreach ($product_variant_ids as $index => $product_variant_id) {
        ...
        if ($product_type[$index] == 'regular') {
            if ($product_variant->stock_type !== null && $product_variant->stock_type !== '') {
...
function isProductDelivarable($type, $type_id, $product_id, $product_type = '', $seller_city_id = '', $seller_zipcode_id = '')
{
    if ($type == 'zipcode') {
        $zipcode_id = $type_id;
    } elseif ($type == 'area') {
    ...
    if (!empty($zipcode_id) && $zipcode_id != 0) {
        ...
        $product = $model::join('seller_store', $table . '.seller_id', '=', 'seller_store.seller_id')
            ...
            ->where(function ($query) use ($zones_serviceable_zipcodes, $zipcode_id) {
                $query->where(function ($subquery) use ($zones_serviceable_zipcodes) {
                    $subquery->where('deliverable_type', '2')
                        ->whereIn('deliverable_zones', $zones_serviceable_zipcodes);
                })
                    ->orWhere('deliverable_type', '1')
                    ->orWhere(function ($subquery) use ($zones_serviceable_zipcodes) {
                        $subquery->where('deliverable_type', '3')
                            ->whereNotIn('deliverable_zones', $zones_serviceable_zipcodes);
                    });
            })
            ->where('seller_store.zipcode', $zipcode_id)
            ->count();
        // dd($product);
        return $product > 0;
        ...
        $product = $model::join('seller_store', $table . '.seller_id', '=', 'seller_store.seller_id')
            ...
            ->where(function ($query) use ($zones_serviceable_cities) {
                $query->where(function ($subquery) use ($zones_serviceable_cities) {
                    $subquery->where('deliverable_type', '2')
                        ->whereIn('deliverable_zones', $zones_serviceable_cities);
                })
                    ->orWhere('deliverable_type', '1')
                    ->orWhere(function ($subquery) use ($zones_serviceable_cities) {
                        $subquery->where('deliverable_type', '3')
                            ->whereNotIn('deliverable_zones', $zones_serviceable_cities);
                    });
            })
            ->where('seller_store.city', $city_id)
            ->count();
...
function placeOrder($data, $for_web = '')
{
...
function validatePromoCode($promo_code, $user_id, $final_total, $for_place_order = 0)
{
    // dd($promo_code);
...
function checkCartProductsDeliverable($user_id, $zipcode = "", $zipcode_id = "", $store_id = '', $city = "", $city_id = "", $is_saved_for_later = 0)
{
    // dd($store_id);
    // echo "here";
    // echo $zipcode_id;
            ...
            if (isset($settings['local_shipping_method']) && $settings['local_shipping_method'] == 1) {
                ...
                if (isset($deliverabilitySettings[0]->product_deliverability_type) && !empty($deliverabilitySettings[0]->product_deliverability_type)) {
                    if ($deliverabilitySettings[0]->product_deliverability_type == 'city_wise_deliverability') {
                        ...
                        $tmpRow['is_deliverable'] = (!empty($city_id) && $city_id > 0) ?
                            isProductDelivarable('city', $city_id, $cart[$i]->product_id, $cart[$i]->cart_product_type, $seller_city_id)
                            : false;
                    } else {
                        // dd('here');
                        // dd($seller_zipcode_id);
                        $tmpRow['is_deliverable'] = !empty($zipcode_id) && $zipcode_id > 0 ?
                            isProductDelivarable('zipcode', $zipcode_id, $cart[$i]->product_id, $cart[$i]->cart_product_type, '', $seller_zipcode_id) :
                            false;
                    }
                }
        ...
        if (count($res) >= 1) {
            ...
            foreach ($res as $row) {
                ...
                if ($currentStatus == 'delivered') {
                    ...
                    $order_final_total = fetchDetails('orders', ['id' => 464], ['delivery_charge', 'total', 'final_total', 'payment_method', 'promo_discount', 'is_cod_collected', 'wallet_balance']);
...
function fetchOrderItems($orderItemId = null, $userId = null, $status = null, $deliveryBoyId = null, $limit = 25, $offset = '0', $sort = null, $order = null, $startDate = null, $endDate = null, $search = null, $sellerId = null, $orderId = null, $store_id = "")
{
...
function getDeliveryBoys($id, $search, $offset, $limit, $sort, $order, $seller_city = '', $seller_zipcode = '', $store_deliverability_type = '')
{
    $multipleWhere = '';
    $where = [];
    $where['u.role_id'] = 3; // Role ID for delivery boys
    ...
    $count_res = DB::table('users as u')
        ->select(DB::raw('COUNT(u.id) as total'), 'c.name as city_name')
        ->leftJoin('cities as c', 'u.city', '=', 'c.id')
        ->leftJoin('zones as z', 'u.serviceable_zones', '=', 'z.id')
        ->where(function ($query) use ($seller_city, $seller_zipcode, $store_deliverability_type) {
            if ($store_deliverability_type == 'city_wise_deliverability') {
                $query->whereRaw("FIND_IN_SET(?, z.serviceable_city_ids)", [$seller_city])
                    ->orWhere('z.serviceable_city_ids', 'LIKE', "%,$seller_city,%");
            } elseif ($store_deliverability_type == 'zipcode_wise_deliverability') {
                $query->whereRaw("FIND_IN_SET(?, z.serviceable_zipcode_ids)", [$seller_zipcode])
    ...
    $delivery_boy_search_res = $search_res
        ->orderBy($sort, $order)
        ->skip($offset)
        ->take($limit)
        ->get();
...
function verifyPaymentTransaction($transaction_id = '', $payment_method = '', $additional_data = [])
{
    ...
    switch ($payment_method) {
        ...
        case 'paystack':
            ...
            $payment = $paystack->verify_transation($transaction_id);
...
function createRow($product, $variant, $category_name, $from_seller = '')
{
    ...
    $tempRow = [
        ...
        'name' => '<div class="d-flex align-items-center"><a href="' . getMediaImageUrl($product_image) . '" data-lightbox="image-' . $variant->id . '"><img src=' . $product_image . ' class="rounded mx-2"></a><div class="ms-2"><p class="m-0">' . $product->name . '</p><p>' . (isset($variant->variant_values) ? '(' . str_replace(",", ", ", $variant->variant_values) . ')' : '') . '</p></div></div>',
        'operate' => $action
    ];
    return $tempRow;
}
...
function getCityNamesFromIds($cityIds)
{
    $cityIdsArray = explode(',', $cityIds);

    return DB::table('cities')->whereIn('id', $cityIdsArray)->pluck('name')->toArray();
}
...
function getZones($request)
{
    ...
    $response_data = $zones->map(function ($zone) use ($cities, $zipcodes) {
        ...
        return [
            'zone_id' => $zone->id,
            'zone_name' => $zone->name,
            'cities' => collect($city_ids)->map(function ($city_id) use ($cities) {
...
function ViewParcel($request, $orderId = null, $sellerId = null, $deliveryBoyId = null)
{
    $offset = $request->input('offset', 0);
    ...
    foreach ($parcels as $parcel) {
        ...
        $itemDetails = DB::table('parcel_items as pi')
            ...
            ->select('oi.*', 'u.username', 'pc.active_status as active_status', 'pi.*', 'ss.store_name', 'o.payment_method', 'o.mobile', 'p.image', 'o.created_at')
            ->get();
        // Create order link
        ...
        foreach ($itemDetails as $itemDetail) {
            $productNames[] = $itemDetail->product_name;
...
function getReturnOrderItemsList($deliveryBoyId = null, $search = "", $offset = 0, $limit = 10, $sort = "oi.id", $order = 'ASC', $sellerId = null, $fromApp = '0', $orderItemId = '', $isPrint = '0')
{
    $filters = [
        'un.username' => $search,
        'u.username' => $search,
    ...
    $applyFilters = function ($query) use ($filters, $deliveryBoyId, $sellerId, $orderItemId) {
        if (!empty($filters)) {
