...
class ComboProductController extends Controller
{
    public function index()
    {
        ...
        return view('seller.pages.forms.combo_products', compact('attributes', 'product_deliverability_type', 'pickup_locations'));
    }
    ...
    public function update(Request $request, $data, $from_app = false)
    {
        $user_id = Auth::user()->id;
        $seller_id = Seller::where('user_id', $user_id)->value('id');
        ...
        $product_data = fetchComboProduct('', '', $data);
    ...
    public function list()
    {
        ...
        if (request()->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('combo_products.id', $search)
        ...
        $combo_products = $combo_products->map(function ($p) {
            ...
            return [
                'id' => $p->id,
                'title' => $p->title . '<br><small>' . ucwords(str_replace('_', ' ', $p->product_type)) . '</small><br><small> By </small><b>' . $p->store_name . '</b>',
    ...
    public function edit($data)
    {
        ...
        if (
            ...
            return view('seller.pages.forms.update_combo_product', compact('data', 'attributes', 'shipping_data', 'combo_product_details', 'product_details', 'product_deliverability_type'));
        }
    }
    ...
    public function show($id)
    {
        ...
        if ($data === null || empty($data)) {
            ...
            return view('seller.pages.views.combo_product', compact('data', 'attributes', 'sellers', 'taxes',  'shipping_data', 'product_faqs', 'product_details', 'combo_product_details', 'rating'));
        }
    }
