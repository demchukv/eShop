...
class ApiController extends Controller
{
        ...
        if ($validator->fails()) {
            ...
            if (Auth::attempt($credentials)) {
                ...
                for ($i = 0; $i < count($store_data); $i++) {
                    ...
                    $store_data[$i]->city = $store_data[$i]->city ? City::where('id', $store_data[$i]->city)->value('name') : '';
                    ...
                    $output['store_data'][] = (array) $store_data[$i];
                }
                for ($k = 0; $k < count($seller_data); $k++) {
                    ...
                    $output['seller_data'][] = (array) $seller_data[$k];
                }
    ...
    public function update_user(Request $request, SellerController $sellerController)
    {
        ...
        */
        ...
        $userData = [
            ...
            'email' => $user->email ?? '',
            'mobile' => $user->mobile ?? '',
            'image' => asset(config('constants.SELLER_IMG_PATH') . $user->image),
            'balance' => $user->balance ?? '0',
            'activation_selector' => $user->activation_selector ?? '',
            'activation_code' => $user->activation_code ?? '',
            'forgotten_password_selector' => $user->forgotten_password_selector ?? '',
        ...
        if ($validator->fails()) {
            ...
            if ($user) {
                ...
                for ($i = 0; $i < count($seller_data); $i++) {
                    unset($seller_data[$i]->created_at);
                    unset($seller_data[$i]->updated_at);

                    $isPublicDisk = $store_data[0]->disk == 'public' ? 1 : 0;

                    $seller_data[$i]->national_identity_card = $isPublicDisk ? asset(config('constants.SELLER_IMG_PATH') . $seller_data[$i]->national_identity_card) : $seller_data[$i]->national_identity_card;
                    $seller_data[$i]->authorized_signature = $isPublicDisk ? asset(config('constants.SELLER_IMG_PATH') . $seller_data[$i]->authorized_signature) : $seller_data[$i]->authorized_signature;
                    $output['seller_data'][] = (array) $seller_data[$i];
                }
                for ($k = 0; $k < count($store_data); $k++) {

                    $store_data[$k]->city_id = $store_data[$k]->city ?? "";
                    $store_data[$k]->city = $store_data[$k]->city ? City::where('id', $store_data[$k]->city)->value('name') : '';
                    $store_data[$k]->zipcode_id = $store_data[$k]->zipcode ?? "";
                    $store_data[$k]->zipcode = $store_data[$k]->zipcode ? Zipcode::where('id', $store_data[$k]->zipcode)->value('zipcode') : "";

                    unset($store_data[$k]->created_at);
                    unset($store_data[$k]->updated_at);

                    $isPublicDisk = $store_data[0]->disk == 'public' ? 1 : 0;

                    $store_data[$k]->logo = $isPublicDisk ? asset(config('constants.SELLER_IMG_PATH') . $store_data[$k]->logo) : $store_data[$k]->logo;
                    $store_data[$k]->store_thumbnail = $isPublicDisk ? asset(config('constants.SELLER_IMG_PATH') . $store_data[$k]->store_thumbnail) : $store_data[$k]->store_thumbnail;
                    $store_data[$k]->address_proof = $isPublicDisk ? asset(config('constants.SELLER_IMG_PATH') . $store_data[$k]->address_proof) : $store_data[$k]->address_proof;
                    $store_data[$k]->other_documents = $isPublicDisk
                        ? (is_array($decoded = json_decode((string) $store_data[$k]->other_documents, true)) ? array_map(fn($document) => asset(config('constants.SELLER_IMG_PATH') . '/' . $document), $decoded) : [])
                        : (json_decode((string) $store_data[$k]->other_documents, true) ?: []);
                    $store_data[$k]->permissions = json_decode($store_data[$k]->permissions, true);
                    $output['store_data'][] = (array) $store_data[$k];
                }
    ...
    public function get_order_items(Request $request)
    {
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            $order_details = fetchOrderItems($id, $userId, $multipleStatus, false, $limit, $offset, $sort, $order, $start_date, $end_date, $search, $seller_id, $order_id, $store_id);
    ...
    public function get_categories(Request $request, CategoryController $categoryController)
    {
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            $request['seller_id'] = $seller_id;
            $cat_res = $categoryController->get_seller_categories($request);
    ...
    public function get_products(Request $request)
    {
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            $products = fetchProduct($user_id, (isset($filters)) ? $filters : $id, $product_id, $category_id, $limit, $offset, $sort, $order, null, null, $seller_id, '', $store_id, $is_detailed_data, $type, 1);
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            $result = DB::table('categories as c')
                ->selectRaw('c.name as name, count(c.id) as counter')
                ->where('p.status', 1)
                ->where('c.status', 1)
            ...
            $tempRow = [
                'cat_name' => $result->pluck('name')->toArray(),
    ...
    {
        $taxes = DB::table('taxes')->select('id', 'title', 'percentage', 'status')->where('status', 1)->get()->toArray();
        ...
        if ($taxes) {
            ...
            $response['error'] = true;
            $response['message'] = 'Taxes does not exist!';
    ...
    public function add_products(Request $request, ProductController $productController)
    {
       ...
       */
        ...
        if ($validator->fails()) {
            ...
            $product = $productController->store($request, true);
    ...
    public function update_products(Request $request, ProductController $productController)
    {
       ...
       */
        ...
        if ($validator->fails()) {
            ...
            $request['extra_input_description'] = (isset($request['extra_input_description']) && $request['extra_input_description'] != 'NULL' && !empty($request['extra_input_description']) ? $request['extra_input_description'] : '');
            $request['pickup_location'] = (isset($request['pickup_location']) && $request['pickup_location'] != 'NULL' && !empty($request['pickup_location']) ? $request['pickup_location'] : '');
            $product = $productController->update($request, $request['edit_product_id'], true);
    ...
    public function get_delivery_boys(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $seller_store = DB::table('seller_store')->where('user_id', $user_id)->where('store_id', $store_id)->select('city', 'zipcode')->get();
            ...
            $data = getDeliveryBoys($id, $search, $offset, $limit, $sort, $order, $seller_city, $seller_zipcode, $store_deliverability_type);
            return response()->json($data);
        }
    }
    ...
    public function upload_media(Request $request, MediaController $mediaController)
    {
        ...
        */
        $validator = Validator::make($request->all(), [
            ...
            'documents.required' => 'Upload at least one media file !', // Custom message for the "required" rule
        ]);
    ...
    public function get_product_faqs(Request $request)
    {
        ...
        if (count($queries) > 0) {
            ...
            $faqs = $faqs->map(function ($faq) use ($faq_type) {
                $faq->type = $faq_type;
                return $faq;
            });
    ...
    public function edit_product_faq(Request $request, ProductFaqController $productFaqController)
    {
        ...
        */
        $validator = Validator::make($request->all(), [
            ...
            'answer' => 'required',
            'type' => 'required', // Ensure type is provided
        ]);
    ...
    public function generate_invoice(Request $request)
    {
        ...
        */
        $validator = Validator::make($request->all(), [
            'shiprocket_order_id' => 'required|numeric',
        ]);
    ...
    public function get_seller_stores()
    {
        $user = Auth::user();
        $user_id = auth()->user()->id;
        $seller_id = Seller::where('user_id', $user_id)->value('id');
    ...
    public function get_combo_products(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $store_id = $request->input('store_id') ? (int) $request->input('store_id') : '';
            $products = fetchComboProduct('', $filters, $id, $limit, $offset, $sort, $order, '', '', $seller_id, $store_id, $category_id, $brand_id, $type, 1);
    ...
    public function add_combo_product(Request $request, ComboProductController $ComboProductController)
    {
        ...
        if ($validator->fails()) {
            ...
            $product_data = $ComboProductController->store($request, true);
    ...
    public function update_combo_product(Request $request, ComboProductController $ComboProductController)
    {
        ...
        if ($validator->fails()) {
            ...
            $request['similar_product_id'] = isset($request['similar_product_ids']) ? explode(",", $request['similar_product_ids']) : "";
            $request['other_images'] = (isset($request['other_images']) && !empty($request['other_images'])) ? explode(",", $request['other_images']) : NULL;
            $product_data = $ComboProductController->update($request, $id, true);
            if (!empty($product_data)) {
                return response()->json([
                    'error' => false,
    ...
    public function get_languages(Request $request)
    {
        ...
        $languages = Language::select('id', 'language', 'code', 'is_rtl')->get(); // You can adjust this as per your requirement, e.g., Language::select('id', 'name')->get();
    ...
    public function top_selling_products(Request $request)
    {
        ...
        $query = DB::table('order_items')
            ->join('product_variants', 'order_items.product_variant_id', '=', 'product_variants.id')
            ->join('products', 'product_variants.product_id', '=', 'products.id')
            ->select('products.image as image', 'products.category_id', 'products.brand as brand_id', 'products.name', DB::raw('SUM(order_items.quantity) as total_sold'))
            ->where('order_items.seller_id', '=', $seller_id)
            ->where('order_items.store_id', '=', $store_id);
