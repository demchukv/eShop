...
class ProductController extends Controller
{
    public function index()
    {
        ...
        return view('seller.pages.forms.products', compact('attributes', 'pickup_locations', 'brands', 'product_deliverability_type'));
    }
    ...
    public function store(Request $request, $fromApp = false)
    {
        ...
        if (request()->has('product_type') && request()->input('product_type') == 'simple_product') {
            ...
            $validator->sometimes('variant_dealer_price.*', 'nullable|numeric', function ($input) {
                return true;
            });
        ...
        if ($validator->fails()) {
            ...
            $product_data = fetchProduct('', $filter, $product->id);
    ...
    public function edit($data)
    {
        ...
        if ($data === null || empty($data)) {
            ...
            return view('seller.pages.forms.update_product', compact('data', 'attributes', 'sellers', 'brands', 'product_variants', 'country', 'shipping_data', 'brand_name', 'product_deliverability_type'));
        }
    }

    public function update(Request $request, $data, $fromApp = false)
    {

        $product_details = fetchdetails('products', ['id' => $data], ['name', 'slug', 'seller_id', 'status']);
        $store_id = !empty(request('store_id')) ? request('store_id') : getStoreId();
        $user_id = Auth::id();
        ...
        if ($validator->fails()) {
            ...
            $product_data = fetchProduct('', '', $data, from_seller: 1);
            $product_data = isset($product_data['product']) && !empty($product_data['product']) ? $product_data['product'][0] : [];
    ...
    public function show($id)
    {
        ...
        if ($data === null || empty($data)) {
            ...
            return view('seller.pages.views.product', compact('data', 'attributes', 'sellers', 'taxes', 'brands', 'product_variants', 'country', 'shipping_data', 'brand_name', 'categories', 'rating', 'product_faqs', 'sales_count', 'attribute_values'));
        }
    }
