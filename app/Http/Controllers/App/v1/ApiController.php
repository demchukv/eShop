...
*/
    ...
    public function get_categories(CategoryController $categoryController, Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $cat_res = $categoryController->get_categories($id, $limit, $offset, $sort, $order, $has_child_or_item, '', '', '', $store_id, $search, $ids);
            $popular_categories = $categoryController->get_categories(NULL, "", "", 'c1.clicks', 'DESC', 'false', "", "", "", $store_id);
    ...
    public function get_cities(AreaController $areaController, Request $request)
    {
       ...
       */
        ...
        if ($validator->fails()) {
            ...
            $order = $request->filled('order') ? $request->input('order') : 'ASC';
            $search = $request->filled('search') ? trim($request->input('search')) : '';
            $city_res = $areaController->getCitiesList($sort, $order, $search, $limit, $offset);
    ...
    public function get_products(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $products = fetchProduct($user_id, (isset($filters)) ? $filters : null, $product_id, $category_id, $limit, $offset, $sort, $order, null, $zipcode, $seller_id, $brand_id, $store_id, $is_detailed_data, $type);
    ...
    public function get_combo_products(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $products = fetchComboProduct($user_id, $filters, $product_id, $limit, $offset, $sort, $order, '', $zipcode, $seller_id, $store_id, $category_id, $brand_id, $type);
            $filtered_brand_ids = array_filter($products['brand_ids'], function ($value) {
                return !empty($value);
            });
            $brand_ids = implode(',', $filtered_brand_ids);
        ...
        if ($validator->fails()) {
            ...
            for ($i = 0; $i < count($res); $i++) {
                ...
                if (strtolower($res[$i]->type) == 'categories') {
                    ...
                    $id = (!empty($res[$i]->type_id) && isset($res[$i]->type_id)) ? $res[$i]->type_id : '';
                    $combo_pro_res = fetchComboProduct('', '', $id);
        ...
        */
        ...
        foreach ($products as $item) {
            if ($item->product_type == 'regular') {
                ...
                $proDetails = fetchComboProduct($user_id, null, $item->product_id ?? null);
            }
            ...
            if (!empty($tmp_cart_user_data)) {
                for ($i = 0; $i < count($tmp_cart_user_data); $i++) {
                    ...
                    if (!empty($product_data->product_id)) {
                        ...
                        if ($tmp_cart_user_data[$i]->cart_product_type == 'regular') {
                            $pro_details = fetchProduct(request()->input('user_id'), null, $product_data->product_id);
                        } else {
                            $pro_details = fetchComboProduct(request()->input('user_id'), null, $product_data->product_id);
                        }
            ...
            if (!$sections->isEmpty()) {
                foreach ($sections as &$section) {
                    if ($section->product_type == 'custom_combo_products') {
                        ...
                        $products = fetchComboProduct($user_id, '', $comboproductIds, $limit, $offset, $p_sort, $p_order);
                        $response = [
                            'error' => false,
                        ...
                        $section->title = preg_replace('/[^A-Za-z0-9\s&]/', '', $section->title);
                        $section->short_description = htmlspecialchars($section->short_description);
                        ...
                        $products = fetchProduct($user_id, $filters, $productIds, $categories, $p_limit, $p_offset, $p_sort, $p_order, null, $zipcode, null);
                        if (!empty($products['product'])) {
                            $response = [
                                'error' => false,
                                'message' => 'Sections retrieved successfully.',
    ...
    public function validate_promo_code(Request $request)
    {
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            $res = validatePromoCode($promo_code, $user_id, $final_total);
        ...
        if ($validator->fails()) {
            ...
            foreach ($productVariant as $variant) {
                ...
                if ($variant->is_attachment_required && empty($request->file('order_attachment'))) {
                    $response = [
                        'error' => true,
                        'message' => 'Order attachment is required for product ' . $variant->product_name . ' and no files were provided.',
                        'code' => 102,
                    ];
                    return response()->json($response);
                }
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            if (addToCart($request->toArray(), $check_status, true)) {
                ...
                $cart_user_data = $cartController->get_user_cart($user_id, $saved_for_later, '', $store_id);
                    ...
                    foreach ($tmpCartUserData as $index => $cartItem) {
                        ...
                        if (!empty($productData) && !empty($productData->product_id)) {
                            if ($cartItem->cart_product_type == 'regular') {
                                ...
                                $proDetails = fetchComboProduct(request()->input('user_id'), null, $productData->product_id);
                            }
    ...
    public function get_orders(Request $request)
    {
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            $store_id = request('store_id') != null ? request('store_id') : "";
            // dd($order_id);
            $order_details = fetchOrders($order_id, $user_id, $multiple_status, NULL, $limit, $offset, $sort, $order, $download_invoice, $start_date, $end_date, $search, NULL, NULL, NULL, '', true, $store_id);
    ...
    public function is_product_delivarable(Request $request)
    {
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            if ($isPincode) {
                $zipcode_id = fetchDetails('zipcodes', ['zipcode' => $zipcode], 'id');
                $is_available = isProductDelivarable('zipcode', $zipcode_id[0]->id, $product_id, $product_type, '', $seller_zipcode_id);
                ...
                $is_available = isProductDelivarable('city', $city_id[0]->id, $product_id, $product_type, '', $seller_city_id);
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            if (!empty($area_id) || !empty($zipcode_id) || !empty($city_id)) {
                ...
                if (isset($settings[0]->product_deliverability_type) && !empty($settings[0]->product_deliverability_type)) {
                    if ($settings[0]->product_deliverability_type == 'city_wise_deliverability') {
                        $product_delivarable = checkCartProductsDeliverable($user_id, '', '', $store_id, $city, $city_id[0]->id ?? '');
                    } else {

                        $product_delivarable = checkCartProductsDeliverable($user_id, $zipcode, $zipcode_id[0]->id ?? '', $store_id);
                    }
                }
    ...
    public function get_promo_codes(Request $request, PromoCodeController $PromoCodeController)
    {
        ...
        */
        ...
        if ($validator->fails()) {
            ...
            $data = $PromoCodeController->getPromoCodes($limit, $offset, $sort, $order, $search, $store_id);
    ...
    public function get_stores(Request $request, StoreController $StoreController)
    {
        ...
        if ($validator->fails()) {
            ...
            $data = $StoreController->getStores($limit, $offset, $sort, $order, $search);
    ...
    public function validate_refer_code(Request $request)
    {
        ...
        $validator->setCustomMessages([
            'unique' => 'This code is already used by some other user.',
        ]);
    ...
    public function get_notifications(Request $request, NotificationController $NotificationController)
    {
        ...
        $res = $NotificationController->get_notifications($offset, $limit, $sort, $order, $user_id);
        return response()->json([
    ...
    public function get_categories_sliders(Request $request, CategoryController $CategoryController)
    {
        ...
        if ($sliders) {
            foreach ($sliders as $slider) {
                ...
                $sliders[$i]['banner_image'] = getMediaImageUrl($slider['banner_image']);  // Use getMediaImageUrl
                $category_data = [];
    ...
    public function most_selling_products(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $top_selling_products->transform(function ($top_selling_product) use ($zipcode) {
    ...
    public function most_popular_products(Request $request)
    {
        ...
        $top_rated_products->transform(function ($top_rated_product) {
    ...
    public function get_similar_products(Request $request)
    {
        ...
        $products = fetchProduct($user_id, '', '', $category_id, $limit, $offset, $sort, $order, null, '', '', '', '', '1');
        if (!empty($products['product'])) {
            $response = [
                'error' => false,
                'message' => 'Products retrieved successfully!',
    ...
    public function get_combo_similar_products(Request $request)
    {
        ...
        $similar_products = fetchComboProduct('', '', '', $limit, $offset, $sort, $order, '', '', '', $store_id, $category_id);
        return response()->json([
            'error' => false,
    ...
    public function get_most_searched_history(Request $request)
    {
        ...
        // Validate both search term and store ID
        $validator = Validator::make($request->all(), [
