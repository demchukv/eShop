...
{
    public function get_user_cart($user_id, $is_saved_for_later = 0, $product_variant_id = '', $store_id = null)
    {
    ...
    function add_to_cart(Request $request, $check_status = true)
    {
        ...
        $validator = Validator::make($validated_data, $rules);
        ...
        if (!empty($product_availability) && $productType != "digital_product") {
            if ($product_availability == false || $product_availability[0]['is_deliverable'] == false) {
                ...
                if ($request['payment_method'] == "razorpay") {
                    ...
                    $payment = $paystack->verify_transation($request['paystack_reference']);
                    if (!empty($payment)) {
