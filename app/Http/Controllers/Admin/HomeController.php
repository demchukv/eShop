...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // Отримуємо заявки на зміну статусу
        $pendingStatusRequests = UserStatus::with('user')
            ->where('status', 'pending')
            ->orderBy('created_at', 'desc')
            ->take(5)
            ->get();

        return view('admin.pages.forms.home', compact('order_counter', 'id', 'store_id', 'user_counter', 'delivery_boy_counter', 'currency', 'top_sellers', 'total_products', 'top_categories', 'total_store', 'total_seller', 'total_earnings', 'role_id', 'store_details', 'primary_colour', 'messengerColor', 'dark_mode', 'sales', 'pendingStatusRequests'));
    }
