...
class Delivery_boyController extends Controller
{
        ...
        if (request()->has('edit_id')) {
            ...
            if (isset($userData['serviceable_zones']) &&  $userData['serviceable_zones'] != NULL) {
                ...
                foreach ($zones_name as $zone) {
                    $city_names = getCityNamesFromIds($zone->serviceable_city_ids);
    ...
    public function store(Request $request, $fromApp = false)
    {
        ...
        // If from the app, make bonus_type required
        if ($fromApp == true) {
            $validator->sometimes('bonus_type', 'required', function ($input) {
                return true;
            });
        }

        // Validate bonus_amount or bonus_percentage based on the bonus_type
        ...
        // Validate percentage to be between 1 and 100
    ...
    public function list(Request $request)
    {
        ...
        $delivery_boys = $delivery_boys->map(function ($b) {
            ...
            $b->serviceable_zones = $this->get_zones_by_ids($b->serviceable_zones);
    ...
    public function get_zones_by_ids($zone_ids)
    {
        ...
        $zones_array = $zones->pluck('name')->toArray();
        $comma_seprated_zones = implode(',', $zones_array);

        return $comma_seprated_zones;
    }
