...
class ProductController extends Controller
{
    public function index()
    {
        ...
        return view('admin.pages.forms.products', compact('attributes', 'sellers', 'brands', 'product_deliverability_type'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (request()->has('product_type') && request()->input('product_type') == 'simple_product') {
            ...
            $validator->sometimes('dealer_price', 'nullable|numeric|gte:0|lte:' . request()->input('dealer_price') . '|string', function ($input) {
                return true;
            });
            ...
            if (request()->has('variant_stock_level_type') && request()->input('variant_stock_level_type') == "product_level") {
                ...
                $validator->sometimes('variant_dealer_price.*', 'nullable|numeric|gte:0|lte:' . request()->input('variant_dealer_price.*') . '', function ($input) {
                    return true;
                });
            }
        }
        ...
        if ($validator->fails()) {
            ...
            $data = [
                'name' => isset($request->pro_input_name) ? $request->pro_input_name : '',
            ...
            if ($request->product_type == 'simple_product') {
                $product_variant_data = [
                    ...
                    'dealer_price' => isset($request->dealer_price) ? $request->dealer_price : '', // Додаємо dealer_price
                ...
                if (isset($request->variant_stock_status) && $request->variant_stock_status == '0') {
                    if ($request->variant_stock_level_type == "product_level") {
                        ...
                        $variant_dealer_price = (isset($request->variant_dealer_price) && !empty($request->variant_dealer_price)) ? $request->variant_dealer_price : '0';
                ...
                if (!empty($request->variants_ids)) {
                    ...
                    for ($i = 0; $i < count($variants_ids); $i++) {
                        ...
                        if ($flag == "variant_level") {
                            ...
                            $product_variant_data['dealer_price'] = (isset($variant_dealer_price[$i]) && !empty($variant_dealer_price[$i])) ? $variant_dealer_price[$i] : '0';
            ...
            if ($product->type == 'variable_product') {
                ...
                if ($variant) {
                    $results[] = [
                        ...
                        'text' => $product->name, // Simple product name
                    ];
                }
            }
        }
    ...
    public function edit($data)
    {
        ...
        if ($data === null || empty($data)) {
            ...
            return view('admin.pages.forms.update_product', compact('data', 'attributes', 'sellers', 'brands', 'product_variants', 'country', 'shipping_data', 'brand_name', 'product_deliverability_type'));
        }
    }
    public function update(Request $request, $data)
    {

        $product_details = fetchdetails('products', ['id' => $data], ['name', 'slug']);
                ...
                if (!empty($request->variants_ids)) {
                    ...
                    for ($i = 0; $i < count($variants_ids); $i++) {
                        ...
                        if ($flag == "variant_level") {
                            ...
                            $product_variant_data['price'] = $variant_price[$i];
                            $product_variant_data['special_price'] = (isset($variant_special_price[$i]) && !empty($variant_special_price[$i])) ? $variant_special_price[$i] : '0';
                            $product_variant_data['dealer_price'] = isset($variant_dealer_price[$i]) ? $variant_dealer_price[$i] : ''; // Додаємо dealer_price
                            $product_variant_data['weight'] = $variant_weight[$i];
                            $product_variant_data['height'] = $variant_height[$i];
                            $product_variant_data['breadth'] = $variant_breadth[$i];
                            $product_variant_data['length'] = $variant_length[$i];
        ...
        if ($type == 'upload') {
            while (($row = fgetcsv($handle, 10000, ",")) != FALSE) //get row values
            {
                ...
                if ($temp != 0) {
                    ...
                    // Додаємо перевірку для dealer_price (наприклад, індекс 36)
                    if (!empty($row[36]) && !is_numeric($row[36])) {
                        return response()->json(['error' => 'true', 'message' => 'Dealer price is invalid at row ' . $temp]);
                    }
                    if ($total_variants == 0) {
                        return response()->json([
                            'error' => 'true',
            ...
            while (($row = fgetcsv($handle, 10000, ",")) != FALSE) //get row vales
            {
                if ($temp1 != 0) {
                    ...
                    for ($i = 0; $i < $total_variants; $i++) {
                        ...
                        $index++;
                        $variant_data[$i]['dealer_price'] = isset($row[$index]) && !empty($row[$index]) ? $row[$index] : ''; // Додаємо dealer_price
                        $index++;
                        if (isset($row[$index]) && !empty($row[$index])) {
                ...
                if ($temp1 != 0) {
                    ...
                    for ($i = 0; $i < $total_variants; $i++) {
                        ...
                        if (isset($variant[0]) && !empty($variant[0])) {
                            ...
                            $index++;
                            $variant_data[$i]['dealer_price'] = isset($row[$index]) && !empty($row[$index]) ? $row[$index] : $variant[0]->dealer_price; // Додаємо dealer_price
                            $index++;
                            if (isset($row[$index]) && !empty($row[$index])) {
                                $variant_data[$i]['sku'] = $row[$index];
                            } else {
    ...
    public function show($id)
    {
        ...
        if ($data === null || empty($data)) {
            ...
            return view('admin.pages.views.product', compact('data', 'attributes', 'sellers', 'taxes', 'brands', 'product_variants', 'country', 'shipping_data', 'brand_name', 'categories', 'rating', 'product_faqs', 'sales_count', 'attribute_values'));
        }
    }
