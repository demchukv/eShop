...
class PromoCodeController extends Controller
{
    ...
    {
        return view('admin.pages.forms.promo_code');
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator->after(function ($validator) use ($request) {
            if ($request->input('discount_type') === 'amount') {
                $minimumOrderAmount = $request->input('minimum_order_amount');
                $discount = $request->input('discount');
                $maxDiscountAmount = $request->input('max_discount_amount');

                // Check if discount and max_discount_amount are less than minimum_order_amount
        ...
        if (isset($request->repeat_usage) && $request->repeat_usage == '1') {
            $validator->sometimes('no_of_repeat_usage', 'required', function ($input) {
                return true;
            });
        }
        ...
        $validated_data['promo_code'] = $request->promo_code;
        $validated_data['title'] = $request->title;
        $validated_data['message'] = $request->message;
        $validated_data['start_date'] = $request->start_date;
        $validated_data['end_date'] = $request->end_date;
        $validated_data['image'] = $request->image;
        $validated_data['no_of_users'] = $request->no_of_users;
        $validated_data['minimum_order_amount'] = $request->minimum_order_amount;
        $validated_data['discount'] = $request->discount;
        $validated_data['discount_type'] = $request->discount_type;
        $validated_data['max_discount_amount'] = $request->max_discount_amount;
        $validated_data['repeat_usage'] = $request->repeat_usage;
        $validated_data['no_of_repeat_usage'] = $request->no_of_repeat_usage;
        $validated_data['status'] = $request->status;
        $validated_data['is_cashback'] = ($request->is_cashback != 'null' && $request->is_cashback == "on") ? 1 : 0;
        $validated_data['list_promocode'] = ($request->list_promocode != 'null' && $request->list_promocode == "on") ? 1 : 0;
        $validated_data['store_id'] = $store_id;

        PromoCode::create($validated_data);
    ...
    public function list()
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$promo_code) {
            ...
            $promo_code->update($validated_data);
            if ($request->ajax()) {
                return response()->json([
    ...
    public function getPromoCodes($limit = null, $offset = null, $sort = 'id', $order = 'DESC', $search = null, $store_id)
    {
