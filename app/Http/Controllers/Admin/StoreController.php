...
class StoreController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $store->slug = generateSlug($request->name, 'stores');
    ...
    // {
    //     // Set all stores' default status to 0
    //     Store::query()->update(['is_default_store' => 0]);
    ...
    //     // Check if the store's status is 0
    //     if ($store->status == 0) {
    //         return response()->json([
    //             'error' => true,
    //             'error_message' => labels('admin_labels.deactivate_store_can_not_be_Set_as_default', 'Deactivated store can not be set as default'),
    //         ]);
    //     } else {
    //         // Set the selected store as the default store
    //         $store->is_default_store = 1;
    //         $store->save();
