...
@section('content')
    ...
    <div class="col-xxl-12 p-0 mt-4">
        <div class="card mt-md-4 mt-xxl-4">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h4>{{ labels('admin_labels.pending_status_requests', 'Pending Status Requests') }}</h4>
                    <a href="{{ route('admin.user_status.index') }}" class="view_all">{{ labels('admin_labels.view_all', 'View All') }}</a>
                </div>
                @if(isset($pendingStatusRequests) && count($pendingStatusRequests) > 0)
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>{{ labels('admin_labels.user', 'User') }}</th>
                                    <th>{{ labels('admin_labels.requested_role', 'Requested Role') }}</th>
                                    <th>{{ labels('admin_labels.message', 'Message') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($pendingStatusRequests as $pendeingRequest)
                                    <tr>
                                        <td>{{ $pendeingRequest->user->first_name }} {{ $pendeingRequest->user->last_name }}</td>
                                        <td>{{ ucfirst($pendeingRequest->type) }}</td>
                                        <td>{{ ucfirst($pendeingRequest->message) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                @else
                    <div class="text-center py-3">
                        <p>{{ labels('admin_labels.no_pending_requests', 'No pending requests') }}</p>
                    </div>
                @endif
            </div>
        </div>
