...
@section('content')
    <section class="main-content">
        <div class="row">
            <div class="d-flex row align-items-center">
                <div class="col-md-6 page-info-title">
                    <h3>{{ labels('admin_labels.order_details', 'Order Details') }}</h3>
                    <p class="sub_title">
                        {{ labels('admin_labels.every_detail_at_your_fingertips', 'Every detail at your fingertips') }}
                    </p>
                </div>
                <div class="col-md-6 d-flex justify-content-end">
                    <nav aria-label="breadcrumb" class="float-end">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                {{ labels('admin_labels.order', 'Order') }}
                            </li>
        ...
        <section>
            <div class="card content-area p-3 mb-3">
                <div class="align-items-center d-flex justify-content-between">
                    ...
                    <div class="align-items-center d-flex">
                        <span class="body-default text-muted">{{ labels('admin_labels.order_date', 'Order Date') }}
                            :</span>
                        <span class="body-default me-3">
                            {{ date('d M, Y', strtotime($order_detls[0]->created_at)) }}
                        </span>
                        {{-- <a href="{{ route('seller.orders.generatInvoicePDF', $order_detls[0]->order_id) }}"
            ...
            <div class="row mt-5 order-info mb-3">
            ...
            @if ($order_detls[0]->active_status == 'awaiting')
                <div class="card content-area p-3">
                    <p>Order status: {{ $order_detls[0]->active_status }}</p>
                    @if ($order_detls[0]->set_delivery_charge == 0)
                        <p>The customer is waiting for the shipping amount. Please determine the shipping cost and enter it
                            in
                            the
                            field below.</p>
                        <form action="{{ route('seller.orders.set_shipping_amount', $order_detls[0]->order_id) }}"
                            method="POST" id="set_shipping_amount_form"
                            class="row row-cols-lg-auto g-3 align-items-center">
                            @csrf
                            <div class="col-12">
                                <label for="shipping_amount">Delivery Charge:</label>
                            </div>
                            <div class="col-12">
                                <input type="number" class="form-control mb-2 mr-sm-2" id="shipping_amount"
                                    name="shipping_amount" placeholder="Enter delivery charge"
                                    value="{{ $order_detls[0]->delivery_charge }}" required>
                            </div>
                            <div class="col-12">
                                <button type="submit" class="btn btn-primary mb-2"
                                    id="set_shipping_amount_btn">Submit</button>
                            </div>
                        </form>
                    @else
                        <p>You have already set the shipping amount. Payment is expected from the customer.</p>
                        <p>Delivery charge: {{ $order_detls[0]->delivery_charge }}</p>
                    @endif
                                    ...
                                    <table
                                        ...
                                        <thead>
                                            ...
                                            <tr>
                                                ...
                                                <th>{{ labels('admin_labels.attachment', 'Ataachment') }}</th>
            ...
            <div class="row">
                <div class="col-12">
                    <iframe src="{{ url('chatify/' . $order_detls[0]->user_id) }}" id="chat-iframe" height="500"
                        width="500"></iframe>
                </div>
            </div>
                    ...
                    <div class="tab-content" id="pills-tabContent">
                        ...
                        @elseif ($order_detls[0]->is_custom_courier == '1')
                            <div class="tab-pane fade show active" id="pills-aftership" role="tabpanel"
                                ...
                                <div class="card card-info mt-3 manual-status-update-box d-none">
                                    <div class="card-header">
                                        <h6 class="mb-0">
                                            {{ labels('admin_labels.update_order_status', 'Update order status') }}
                                        </h6>
                                    </div>
                                    <form id="manual-status-update-form" method="POST"
                                        action="{{ route('seller.orders.update_order_status') }}">
                                        @csrf
                                        <div class="card-body">
                                            <div class="form-group">
                                                <label
                                                    for="order_status">{{ labels('admin_labels.status', 'Status') }}</label>
                                            ...
                                            <!-- Передаємо parcel_id замість order_id і order_item_id -->
                                            <input type="hidden" name="parcel_id" value="">
                                        </div>

                                        <div class="d-flex justify-content-end p-2">
                                            <button type="submit" class="btn btn-primary" id="update_parcel_status_btn">
                                                {{ labels('admin_labels.update', 'Update status') }}
                                            </button>
                                        </div>
                                    </form>
                                </div>
